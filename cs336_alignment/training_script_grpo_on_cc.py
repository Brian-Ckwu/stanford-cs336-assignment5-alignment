...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an LLM with GRPO on Capacility Calibration.")
...
for i in tqdm(range(num_rollout_steps), desc="GRPO training steps"):
    ...
    print(f"Generating rollouts for the following {len(train_rows)} questions (answers): ", [train_row["query"].split()[0] + f" ({train_row['answer']})" for train_row in train_rows])

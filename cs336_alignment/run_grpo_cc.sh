...
uv run training_script_grpo_on_cc.py \
  ...
  --wandb-exp-name "${model_id}_lr-${lr}_lora-r-16-a-32-dropout-0-bf16_runpod_train-6400" \
  --model-id "${model_id}" \
  --dataset_path "../data/capability_calibration/triviaqa-train__Qwen3-8B-non-thinking/grpo_dataset.jsonl" \
  --policy-device 0 \
  --rollout-device 0 \
  --gpu-memory-utilization 0.18 \
  --prompt-path "prompts/verbalized_cc_boxed.prompt" \
  --use-chat-template \
  --chat-template-path "chat_templates/qwen3_thinking.jinja" \
  --confidence-output-format boxed \
